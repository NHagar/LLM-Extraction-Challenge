...
def create_combined_dataframe(training_df, json_data):
    ...
    keep_columns = ['name', 'email', 'subject', 'date', 'year', 'month', 'day', 'hour', 'minute', 'domain', 'party', 'disclaimer']
    
    # Create base dataframe with selected columns
    df = training_df[keep_columns].copy()
    
    # Add the canonical committee column (uppercase)
    df['committee'] = training_df['committee'].apply(lambda x: str(x).upper() if pd.notna(x) else None)
    ...
    for model_name, emails in json_data.items():
        ...
        for email in emails:
            ...
            keys_to_try = [
                (email.get('subject', ''), email.get('date', ''), email.get('name', '')),
                (email.get('subject', ''), email.get('date', ''), email.get('email', '')),
                (email.get('subject', ''), str(email.get('year', '')), str(email.get('month', '')), str(email.get('day', ''))),
            ]
        ...
        for idx, row in df.iterrows():
            keys_to_try = [
                ...
                (row.get('subject', ''), str(row.get('year', '')), str(row.get('month', '')), str(row.get('day', ''))),
            ]
    ...
    model_columns = [col for col in df.columns if col not in keep_columns + ['committee']]
